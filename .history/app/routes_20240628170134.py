    ...
    if'user_id' in session:
        ...
        if form.validate_on_submit() and form.submit.data:
            ...
            try:
                ...
                db.session.rollback()
                flash('an error has occured while adding transaction, please try again :{}'.format(str(e)),'danger')
        return render_template('page_version_2.html',title="dashboard",modif=modify,filter_date=month_filter,category=category,Transac=transactions,transactform=form,style='./static/dashboard.css',script='./static/dashboard.js')
    else:
...
def delete():
    if'user_id' in session:
        ...
        return redirect(url_for('page_version_2'))
    else:
        flash('please login','warning')
        return redirect(url_for('homepage'))
